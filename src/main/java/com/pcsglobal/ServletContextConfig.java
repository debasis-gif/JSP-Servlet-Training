...
public class ServletContextConfig extends HttpServlet 
{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException 
	{
		...
		str = ctx.getInitParameter("phone");   // <context-param> in web.xml as separate xml block -> ctx object fetches context parameter name which is phone
		str = "You have " + str + " phone";
		out.println(str);

		// ServletConfig: Need to extract the name and phone from Config Param set in web.xml
		// When we have multiple servlets but want to use the same parameters in all servlets the use ServletContext
...
/*
		...
		String str = cfg.getInitParameter("name");   // <init-param> in web.xml under <servlet> -> cfg object fetches config parameter name which is name
		out.println(str);
		
		str = cfg.getInitParameter("phone");   // <init-param> in web.xml under <servlet> -> cfg object fetches config parameter name which is phone
